...
if (-not (Test-Path -LiteralPath $databasePath -PathType Leaf)) { throw 'Binding replay database does not exist' }
if (-not (Test-Path -LiteralPath $workspacePath -PathType Container)) { throw 'Binding replay workspace does not exist' }
...
try {
    ...
    try {
        ...
        try {
            $raw = @(& cargo test --release -p fossilsense --bin fossilsense 'server::tests::binding_replay::benchmark_uboot_binding_replay' -- --ignored --exact --nocapture 2>&1 | ForEach-Object { $_.ToString() })
            $cargoExit = $LASTEXITCODE
        } finally { $ErrorActionPreference = $savedErrorAction }
    } finally { Pop-Location }
    Assert-ReplayProcessSuccess -ExitCode $cargoExit -Output $raw
    $allowed = @(Get-BindingReplayMetricNames)
    $metrics = [ordered]@{}
    foreach ($line in $raw) {
