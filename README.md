...
普通标识符补全会经过统一的 completion 模块，合并来自不同通道的证据、去重、排序再截断。排序时，文件局部性和 include 可达性作为“软先验”参与，并通过 guard band 阻止低置信度的全局、语言内置或文本噪音轻易反超更可信的候选。同名候选（无论来自索引、局部绑定、当前文件 overlay、语言内置候选还是文本）会被合并为一个可解释的条目，原始文本兜底仍明确标注为 `text`，绝不伪装成语义定义。

项目上下文（project context）会从常见构建标记推断：`Makefile` / `makefile` / `GNUmakefile`、`CMakeLists.txt`、QMake `*.pro`、Visual Studio `*.sln` / `*.vcxproj` / `*.vcproj`。默认明确忽略 Ninja 标记（例如 `build.ninja`），因为它们常位于生成的 build 目录中。自动模式使用当前文件最近的祖先标记目录作为上下文；嵌套项目选择最近标记，多个标记位于同一目录时合并为一个项目。该信号只用于普通标识符补全的有界排序证据和召回代表性，不会过滤其它项目的候选，不会影响跳转、引用、着色或成员补全，也不会解析构建系统。

VS Code 状态栏提供 `Current Project (Auto)`、已发现项目路径和 `Unspecified` 三类选择。手动选择保存在 VS Code `workspaceState`，不会写入仓库文件、全局设置或索引数据库。`Unspecified` 以及 `fossilsense.projectContext.mode = "off"` 是严格 opt-out：普通补全排序与禁用项目上下文时一致。`promptOnAmbiguous` 会在当前文件无法由标记可靠推断项目时提示一次选择。
...
同样地，当前打开文档里未保存的宏、typedef/using alias、枚举常量、函数声明/定义、record/type 定义和附近的 identifier 使用，都会作为 current-file overlay evidence 参与普通补全。
...
| `Select Project Context` | 选择自动项目上下文、某个已发现项目，或 `Unspecified` 以禁用项目上下文排序。 |
