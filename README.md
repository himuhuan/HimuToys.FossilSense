...
* Rust 引擎 (crate)：`crates/fossilsense/Cargo.toml`，版本 `1.2.0`
* VS Code 扩展 (VSIX)：`extensions/vscode/package.json`，版本 `1.2.0`
...
v1.2.0 smart completion 已推进到 Phase 2-3：普通标识符补全现在经过 `completion` 核心模块合并 evidence、去重、排序和截断。Displayed ranking 使用 deterministic evidence-aware ranker：include `ScopeTier` 仍参与排序，但作为 soft prior，而不是 strict packed score 的硬字典序；guard band 会阻止低置信 global/text 噪音轻易反超更可信候选。当前 open document 中未保存的宏、typedef/using alias、枚举常量、函数声明/定义、record/type 定义和附近 identifier 使用也会作为 current-file overlay evidence 参与普通补全。同名 indexed/local/overlay/text 候选会合并为一个可解释 item，raw text fallback 仍标为 `text`，不伪装成语义定义。verbose/perf 日志只输出分阶段耗时、来源/返回计数、guard 摘要和 shadow-rank 摘要，不输出候选名或源码片段。intent classifier、多通道召回、include recent/sibling ranking、成员方法补全、历史个性化、telemetry 和 ML ranker 仍未启用。
