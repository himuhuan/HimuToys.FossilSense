...
* Rust 引擎 (crate)：`crates/fossilsense/Cargo.toml`，版本 `1.4.2`
* VS Code 扩展 (VSIX)：`extensions/vscode/package.json`，版本 `1.4.2`
...
该门禁会确认版本号、release notes、`dist/fossilsense-vscode-1.4.2_BUILD*.vsix`、VSIX 内的 `extension/bin/fossilsense.exe`，并核对包内 release-input SHA-256 与当前源码。交付说明必须精确记录最终 VSIX 文件名、VSIX SHA-256、release-input SHA-256 和打包时 source commit；打包后再改 Rust/扩展/打包关键输入会使旧包失效。直接在 VS Code 中选择 `Install from VSIX` 即可安装。
