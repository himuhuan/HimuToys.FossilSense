...
* Rust 引擎 (crate)：`crates/fossilsense/Cargo.toml`，版本 `1.2.2`
* VS Code 扩展 (VSIX)：`extensions/vscode/package.json`，版本 `1.2.2`
...
该门禁会确认版本号、release notes、`dist/fossilsense-vscode-1.2.2_BUILD*.vsix` 以及 VSIX 内的 `extension/bin/fossilsense.exe`。直接在 VS Code 中选择 `Install from VSIX` 即可安装。
