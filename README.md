...
当前版本：`1.7.1`。本版本改进大仓库中的补全、符号绑定与内存管理：代码位置和名称相同并不足以判定同一个声明；跳转、悬停与补全详情共用有依据的声明关系。索引构建限制并发和解析中的临时数据，声明缓存按访问顺序淘汰；保存少量文件时复用未变化的辅助索引。
...
3. 选择 `fossilsense-vscode-1.7.1_BUILD*.vsix`。
