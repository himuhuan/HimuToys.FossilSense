...
当前版本：`1.6.0`。本版本重点完善纯 C 容错解析：文件级 tag 前置声明可与定义配对，函数内枚举值不再污染工作区索引，GNU weak 函数身份保持一致，protobuf-c 导出宏夹在 `struct` 与类型名之间时仍能恢复真实记录定义，声明边界宏也不再吞掉首个 `typedef`，硬解析失败时也会保留能够安全识别的多个全局变量名。普通补全名称索引同时减少了重复整数和小写字符串存储；在同机 U-Boot 门禁中，预热旧索引后的双代峰值由 501.61 MiB 降至 483.02 MiB，64 次真实补全请求 P95 为 28.823 ms。默认关闭的 protobuf-c 来源追溯和实验性 Go 后端继续提供；FossilSense 仍不执行任意宏展开，也不声称具备完整 C++ 编译器语义。
