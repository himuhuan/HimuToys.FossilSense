...
* Rust 引擎 (crate)：`crates/fossilsense/Cargo.toml`，版本 `1.2.1`
* VS Code 扩展 (VSIX)：`extensions/vscode/package.json`，版本 `1.2.1`
...
verbose/perf 日志只输出分阶段耗时、来源/返回计数、intent bucket、history bucket counts、recall channel counts、guard 摘要、shadow-rank 摘要、member/include ranking 计数，不输出候选名、accepted label、include path 或源码片段。v1.2.1 schema 变化可能在首次启动时触发全量重建；状态栏会继续显示索引进度。
