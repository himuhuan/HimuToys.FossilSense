...
v1.2.0 开始推进 smart completion 的 Phase 0-1：普通标识符补全现在经过一个独立的兼容 pipeline 合并、去重、排序和截断候选，并在 verbose/perf 日志中输出分阶段耗时、候选来源计数和 shadow-rank 摘要。这个阶段的目标是建立可观测和可测试的补全内核；用户可见排序仍保持当前 strict-tier 兼容行为，尚未启用 soft scope prior、intent classifier、ML ranker、历史个性化或成员方法补全。
