...
* Rust 引擎 (crate)：`crates/fossilsense/Cargo.toml`，版本 `1.4.1`
* VS Code 扩展 (VSIX)：`extensions/vscode/package.json`，版本 `1.4.1`
...
同一名字匹配档内，文件局部性和 include 可达性作为“软先验”参与，并通过 guard band 阻止低置信度的全局、语言内置或文本噪音轻易反超更可信的候选。同名候选（无论来自索引、局部绑定、当前文件 overlay、语言内置候选还是文本）会被合并为一个可解释的条目，原始文本兜底仍明确标注为 `text`，绝不伪装成语义定义。
...
该门禁会确认版本号、release notes、`dist/fossilsense-vscode-1.4.1_BUILD*.vsix` 以及 VSIX 内的 `extension/bin/fossilsense.exe`。直接在 VS Code 中选择 `Install from VSIX` 即可安装。
