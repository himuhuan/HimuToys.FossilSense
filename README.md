...
当前版本：`1.7.2`。本版本增加可选择的进程资源档位；资源等待到期会明确提示更新失败和恢复方法。大仓库中的补全、符号绑定与内存管理继续遵循以下规则：代码位置和名称相同并不足以判定同一个声明；跳转、悬停与补全详情共用有依据的声明关系。索引构建限制并发和解析中的临时数据，声明缓存按访问顺序淘汰；保存少量文件时复用未变化的辅助索引。
...
保存文件后，备用名称、include 补全、Go import 补全、头文件路径和文件列表可以共享旧的基础数据，仅替换有界的变化部分。配置、模块关系或数据库实例发生变化，或者累计变化超过预算时，会完整重建相应读取数据。include 候选检查达到上限时会向编辑器报告结果不完整。状态栏内存明细也包含头文件路径索引。
...
3. 选择 `fossilsense-vscode-1.7.2_BUILD*.vsix`。
...
当参数或局部对象的所属类型有明确证据时，字段与直接方法名称的悬停、定义跳转和声明跳转共用成员事实，定位成员自己的位置；字段悬停的注释来自同一文件修订。支持普通对象、指针、有限成员链、成员声明，以及 `struct S s = {.state = 1}` 这类简单初始化器。Go 目前限于有证据的同 package 类型；数组下标、复杂初始化器和未能证明所属类型的表达式不做可靠跳转，成员补全仍可提供探索候选。对于有明确所属类型和兼容签名的方法，定义跳转也会查找类外实现；存在多个合理实现时保留多个位置。升级后旧索引会自动重新建立。

定义跳转、声明跳转、悬停和补全详情共用声明之间的关联证据，支持接口声明与实现、类型前向声明与完整定义，并保留别名自身的位置。只排除有明确互斥证据的条件分支，复杂条件继续视为不确定；静态符号和不同所属类型不会仅因同名而合并。查询规模有上限，超限会保留不完整标记。已选补全详情按声明身份读取，不会因同名候选过多而改指其他位置。
...
完整索引、增量索引、读取模型生成、名称压实和项目上下文刷新共用一个进程级构建额度，首版同一时间只运行一个重型构建。等待其他构建时状态栏会显示 `waiting for resources`，每个工作区只保留最新待处理目标；多工作区中，一个工作区就绪不会掩盖另一个工作区仍在等待、失败或恢复构建的状态。内存不足时最多等待 30 秒；仍无法准入则显示失败，旧索引继续可用但可能过时。失败的工作区会记住需要重新扫描，下一次保存或刷新会补齐此前遗漏的修改和删除。也可释放资源或调整资源档位并重启服务后，执行 Full Rebuild Index 恢复完整更新。新的保存会合并到待处理 dirty 集合，并可协作取消已经过期的后台名称压实；关闭服务器或移除工作区也会停止等待和可取消阶段。等待期间旧索引仍继续响应 Hover、跳转和补全，成功构建的新数据库与读取模型只在最终校验后一起切换。
...
`fossilsense.resources.profile` 为整个服务进程选择资源档位：`conservative` 为 512 MiB、默认 `balanced` 为 1 GiB、`large` 为 2 GiB。应根据机器可用内存和实际仓库选择，修改后重启 FossilSense。命令行可通过 `FOSSILSENSE_RESOURCE_PROFILE` 环境变量选择同名档位。重型构建仍共享 256 MiB 临时预留并保留 32 MiB 未归因空间。这里的预留是准入估算，不是操作系统分配器的硬上限；`semanticIndex.memoryBudgetMB` 仍只表示声明名称索引和详情缓存的局部目标，不包含运行时、文件关系图、打开文档或新旧两代并存的进程峰值。Windows 以 Private Bytes、Linux/macOS 以 RSS 作为完整进程门禁；采样不可用会明确标记，不能按 0 内存放行。
