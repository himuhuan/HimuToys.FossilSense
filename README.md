...
Include path completion 保留 quote/angle 的既有搜索顺序优先级，同时加入 same-directory、sibling/component include edge、当前文件 recent include、basename frequency 和 path depth evidence 做二级排序。成员方法补全、local history、telemetry、ML ranker 和 auto include insertion 仍不属于当前版本。verbose/perf 日志只输出分阶段耗时、来源/返回计数、intent bucket、recall channel counts、guard 摘要、shadow-rank 摘要和 include ranking 计数，不输出候选名、include path 或源码片段。
