...
普通标识符补全会经过统一的 completion 模块，合并来自不同通道的证据、去重、排序再截断。排序时，文件局部性和 include 可达性作为“软先验”参与，并通过 guard band 阻止低置信度的全局或文本噪音轻易反超更可信的候选。同名候选（无论来自索引、局部绑定、当前文件 overlay 还是文本）会被合并为一个可解释的条目，原始文本兜底仍明确标注为 `text`，绝不伪装成语义定义。
