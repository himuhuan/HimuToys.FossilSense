...
Version 1.6.0 improves tolerant C parsing: file-scope tag declarations pair with definitions, function-local enum values stay out of the workspace index, standalone GNU weak attributes no longer split one function identity, protobuf-c export macros between `struct` and a generated type name are recovered, and the hard-failure fallback preserves safely recognized multiple global names. A smaller completion name index reduced the warmed two-generation U-Boot gate from 501.61 MiB to 483.08 MiB on the same machine; 64 production LSP completion requests measured 25.246 ms at P95. Opt-in protobuf-c source tracing remains available. FossilSense still does not expand arbitrary macros or claim complete C++ compiler semantics.
...
The tolerant tree-sitter frontends extract typed declaration facts from C/C++ and Go source, with a conservative lexical fallback for a hard AST failure. For simple C global declarations that it can identify safely, that fallback preserves multiple declared names as lowest-priority completion hints, but still gives those hints no navigable declaration identity. In files resolved as C, file-scope `struct`, `union`, and `enum` forward tags are stored as declarations, so Declaration and Definition can target the forward tag and full definition separately; ordinary type uses and function-local forward tags are not promoted by that rule. C enum values defined inside a function remain available to live local queries but are not stored in the workspace index. A standalone GNU C `__attribute__((weak))` no longer splits one function's declaration and definition identities; presentation signatures retain the source attribute, and ABI attributes remain identity-bearing. Protobuf-c generated headers parsed as C tolerate a single-token export macro between `struct` and the real type name, so fields and type definitions remain attached to the generated type. The local SQLite index stores each declaration's stable ID, name, declaration/definition role, source range, signature, linkage or package identity, conditional guard, and file revision. Hover, navigation, Signature Help, Find All, and workspace symbols all use the same candidate service over those facts, include/package reachability, project evidence, and unsaved-document overlays.
