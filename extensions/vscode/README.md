...
Field and direct-method hover, Definition, and Declaration share owner evidence and target the member token. Hover comments come from the same source revision. Supported cases include explicitly typed local objects and parameters, pointers, bounded member chains, member declarations, and simple designated initializers such as `struct S s = {.state = 1}`. Go member navigation is limited to proven types in the same package. Array subscripts, complex initializers, and unproven owners remain unsupported for navigation; member completion keeps exploratory suggestions. For methods with supported owner and signature evidence, Definition also searches for out-of-class implementations and retains multiple compatible targets. Existing indexes rebuild after upgrading.

Definition, Declaration, hover, and completion details share declaration relationship evidence. Supported cases include interface declarations and implementations, forward tags and complete types, and aliases that retain their own locations. Only proven contradictory conditions are excluded; complex conditions remain uncertain. Static symbols and different owners are not merged merely by name. Queries have fixed budgets and report incomplete results when exhausted. Selected completion details resolve the original declaration identity even when a same-name candidate bucket is full.
...
Install `fossilsense-vscode-1.7.2_BUILD*.vsix` with:
...
`fossilsense.resources.profile` selects a process admission target: `conservative` (512 MiB), default `balanced` (1 GiB), or `large` (2 GiB). Choose according to workspace size and available machine memory, then restart FossilSense. CLI users can set `FOSSILSENSE_RESOURCE_PROFILE` to the same values. Heavy builds still share a 256 MiB temporary reservation and retain 32 MiB for unattributed runtime/query overhead. Memory admission waits up to 30 seconds, then reports failure instead of repeatedly queueing the same update. A failed workspace is marked for a full rescan on the next save or refresh, including previously missed edits and deletions. The old index remains queryable but may be stale; free memory or change the profile, restart the server, then run Full Rebuild Index. Reservation is an admission estimate, not an allocator-enforced hard cap. `semanticIndex.memoryBudgetMB` keeps its narrower meaning and does not include runtime overhead, file graphs, open documents, or the process peak while old and new generations coexist. Release gates use Private Bytes on Windows and RSS on Linux/macOS; an unavailable sample is reported explicitly rather than treated as zero.
