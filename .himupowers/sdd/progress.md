...
- Task 7: completed by main agent. RED: `cargo test -p fossilsense completion::tests::history_boost_lifts_comparable_candidate_but_not_current_local -- --nocapture` and `cargo test -p fossilsense server::tests::ordinary_completion_items_attach_history_accept_command_when_enabled -- --nocapture` failed with missing history rank fields/context and missing ordinary completion accept commands. GREEN: `cargo test -p fossilsense completion::tests -- --nocapture`, `cargo test -p fossilsense server::tests -- --nocapture`, and `cargo test -p fossilsense completion_history -- --nocapture` passed. Added bounded history scoring keyed by anonymous candidate hash/kind/intent/prefix bucket, source-safe history perf metrics, ordinary completion accept commands, and disabled-history parity coverage.
- Task 8: completed by main agent. GREEN: docs grep for `1.2.1`, `local history`, and `member` showed current docs now mention v1.2.1 member/history boundaries; placeholder scan returned no matches. `pnpm run compile` and `pnpm run test` in `extensions/vscode` passed. Updated README, CLAUDE, extension README, package descriptions, requirements wording, and the implementation plan documentation sync note.
- Task 9: not started.
