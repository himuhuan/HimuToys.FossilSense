...
清理只能发生在当前实施阶段结束后，不能穿插在编码、失败回归、最终验证、review 或返工过程中。只要相关源码仍可能修改、必要验证或 benchmark 尚未完成、reviewer 提案尚未裁决，就必须保留现有日志、数据库和样本，继续复用有效证据；不能为了临时释放空间提前执行清理，导致重复编译或重复测量。

同时满足以下条件，才视为当前阶段结束并进入提交准备：相关源码、测试和配置已经冻结；按修改风险完成了必要验证；reviewer 结论已经裁决且没有待修复项。达到这个状态后，提交前必须执行一次阶段性测试清理，永久删除、不进入回收站的对象包括：`target/verification/`、`target/benchmark/`、`target/extension-smoke/`、`target/benchmark-entrypoint-test-*`，以及本阶段创建的其他临时 SQLite、日志和测试目录。禁止把这一步扩大成删除整个 `target/` 或执行无范围的 `cargo clean`。

`target/debug/`、`target/release/`、Cargo 元数据、`extensions/vscode/node_modules/`、`extensions/vscode/out/` 和 `extensions/vscode/bin/` 属于可复用的编译或依赖缓存，即使阶段结束也默认保留。只有 Rust、Node.js 或 pnpm 工具链版本、目标平台、构建配置、依赖锁版本发生切换，旧缓存已经不兼容或损坏，或者用户明确要求完整清理时，才删除对应缓存；普通源码修改、review 完成、准备提交或切换分支本身都不是清理理由。

大型仓库样本仅在本阶段仍需运行性能场景时保留。阶段结束后，应删除 `samples/` 下被 `.gitignore` 忽略且后续阶段不再使用的外部样本；不能删除仓库跟踪的小型回归样本。`dist/` 中只删除已经过期且不再需要发布或核验的包；待上传或待核验哈希的最终 VSIX，以及已经进入 `docs/benchmark/` 的长期性能结果，不能按临时产物删除。

清理完成后只检查删除目标、磁盘空间和 `git status --short`，确认没有受版本控制的文件被删除或修改。清理本身不使既有验证证据失效，也不触发重新编译、重新测试或重新打包。
...
* 如果已经完成 review 并准备提交，是否按“阶段结束清理”删除阶段性测试产物、保留仍可复用的编译缓存，并确认没有影响受版本控制文件。
