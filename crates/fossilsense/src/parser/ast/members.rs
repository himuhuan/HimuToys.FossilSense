...
pub(super) fn method_member_kind(
...
pub(super) fn declarator_contains_kind(node: tree_sitter::Node<'_>, kind: &str) -> bool {
    if node.kind() == kind {
        return true;
    }
    let mut cursor = node.walk();
...
pub(super) fn function_declarator_is_pointer_like(node: tree_sitter::Node<'_>) -> bool {
...
pub(super) fn simple_owner_method_from_declarator(
    declarator: tree_sitter::Node<'_>,
    source: &str,
...
pub(super) fn function_like_signature(node: tree_sitter::Node<'_>, source: &str) -> String {
    let end = node
...
pub(super) fn anonymous_record_type_node(type_node: tree_sitter::Node<'_>) -> bool {
    matches!(
        type_node.kind(),
...
#[allow(clippy::too_many_arguments)]
pub(super) fn push_member_at_byte(
    members: &mut Vec<MemberDef>,
    record_key: String,
    name: String,
    start_byte: usize,
...
pub(super) fn enclosing_record_declaration(
...
pub(super) fn record_declaration_range(
    record: tree_sitter::Node<'_>,
    source: &str,
    line_starts: &[usize],
) -> SourceRange {
