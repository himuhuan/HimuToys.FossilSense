...
pub(super) fn record_typedef_aliases(text: &str) -> Vec<String> {
...
pub(super) fn starts_with_typedef_keyword(text: &str) -> bool {
...
pub(super) fn looks_like_record_typedef_prefix(chars: &[char]) -> bool {
    let text: String = chars.iter().collect();
    record_keyword_regex().is_match(&text)
}

pub(super) fn first_code_char(chars: &[char], needle: char) -> Option<usize> {
    first_code_char_from(chars, needle, 0)
}

pub(super) fn first_code_char_from(chars: &[char], needle: char, start: usize) -> Option<usize> {
    let mut i = start;
...
pub(super) fn matching_code_delimiter(
    chars: &[char],
    open_pos: usize,
    open: char,
    close: char,
...
pub(super) fn split_top_level_declarators(chars: &[char]) -> Vec<Vec<char>> {
...
pub(super) fn find_double_bracket_close(chars: &[char], start: usize) -> Option<usize> {
    let mut i = start;
    while i + 1 < chars.len() {
...
pub(super) fn matching_open_delimiter(
    chars: &[char],
    close_pos: usize,
    open: char,
    close: char,
...
pub(super) fn trim_char_vec(chars: &mut Vec<char>) {
...
pub(super) fn skip_quoted_chars(chars: &[char], quote_start: usize) -> usize {
...
pub(super) fn include_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| Regex::new(r#"^#\s*include\s+(.+)$"#).expect("include regex"))
}

pub(super) fn macro_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
...
pub(super) fn function_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
...
pub(super) fn typedef_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
...
pub(super) fn record_keyword_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
...
pub(super) fn identifier_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| Regex::new(r#"[A-Za-z_][A-Za-z0-9_]*"#).expect("identifier regex"))
}

pub(super) fn tag_type_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
...
pub(super) fn global_var_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
        Regex::new(r#"([A-Za-z_][A-Za-z0-9_]*)\s*(?:\[[^\]]*\])?\s*(?:=[^;]*)?;"#)
