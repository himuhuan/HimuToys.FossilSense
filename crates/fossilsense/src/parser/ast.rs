...
pub(super) fn collect_ast_index(
    ...
    while let Some(visit) = stack.pop() {
        ...
        {
            collect_object_declarations(node, path, source, line_starts, &mut out.declarations);
        }
...
fn is_namespace_or_file_scope_declaration(node: tree_sitter::Node<'_>) -> bool {
...
fn object_declarator_has_initializer(declarator: tree_sitter::Node<'_>) -> bool {
...
fn object_declarator_shape(declarator: tree_sitter::Node<'_>, source: &str) -> DeclaratorShape {
...
fn unwrap_init_declarator(node: tree_sitter::Node<'_>) -> tree_sitter::Node<'_> {
...
fn node_has_storage_class(node: tree_sitter::Node<'_>, source: &str, expected: &str) -> bool {
    let mut cursor = node.walk();
...
fn node_has_type_qualifier(node: tree_sitter::Node<'_>, source: &str, expected: &str) -> bool {
    let mut cursor = node.walk();
...
fn strip_object_storage_specifiers(prefix: &str) -> String {
    prefix
        .split_whitespace()
        .filter(|token| {
            !matches!(
...
fn namespace_owner(node: tree_sitter::Node<'_>, source: &str) -> Option<String> {
