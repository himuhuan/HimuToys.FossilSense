use super::*;
use crate::completion_history::{candidate_hash, candidate_hash_key, CompletionHistorySnapshot};
...
fn candidate_with_kind(
    ...
    let mut evidence = CandidateEvidence::new(source, tier, ResolutionConfidence::Heuristic, score);
    evidence.kind = kind;
...
fn neutral_history_context_preserves_existing_order() {
    ...
    let disabled =
        run_evidence_aware_pipeline_with_context(candidates, 10, CompletionRankContext::default());
...
#[test]
fn project_context_boost_survives_low_trust_global_cap() {
    let mut same_project = CandidateEvidence::new(
        CandidateSource::Indexed,
        ScopeTier::Global,
        ResolutionConfidence::Fallback,
        900,
    );
    same_project.kind = CompletionCandidateKind::Function;
    same_project.project_score = 350;

    let mut other_project = CandidateEvidence::new(
        CandidateSource::Indexed,
        ScopeTier::Global,
        ResolutionConfidence::Fallback,
        900,
    );
    other_project.kind = CompletionCandidateKind::Function;

    let output = run_evidence_aware_pipeline(
        vec![
            PipelineCandidate::new("get_other_value", other_project, "other"),
            PipelineCandidate::new("get_app_value", same_project, "same"),
        ],
        10,
    );

    assert_eq!(output.items[0].payload, "same");
    assert!(
        output.items[0].evidence.score > output.items[1].evidence.score,
        "same-project evidence should remain visible after low-trust guard"
    );
    assert_eq!(output.metrics.final_rank.guarded_low_trust, 2);
}
