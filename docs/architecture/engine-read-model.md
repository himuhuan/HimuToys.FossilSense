...
segmented name index 的 compact base 也遵守同一边界。delta segment 达到 64 个，或 retained delta slots 超过 base entries 的 25% 后，dirty publication 先发布并释放 publisher，再把 compact build 放到 blocking worker。worker 构建前后都校验它捕获的 `EngineEpoch`；如果期间已有更新，它丢弃结果，不能把旧内容重新发布。压实完成时只推进 `EngineEpoch`，不伪造新的 `SemanticGeneration`，旧请求仍持有原 snapshot。
