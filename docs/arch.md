...
| 功能 | 当前事实来源 | 主要路径 |
| --- | --- | --- |
| Go to Definition | 当前打开文档 live parse + SQLite symbols + ReachScope | `server/definition.rs` -> `store.symbol_read_view().symbols_by_name` -> `query::rank_definitions_into_candidates_with_scope` |
...
当前 fitness 输出：无失败项；无 large-source-file warning；`query/lsp_kinds.rs` 有一个已 allowlist 的 transitional LSP-kind adapter。
...
当前 `scripts/verify_architecture_fitness.ps1 -Format text` 无失败项，也无 large-source-file warning。此前的 completion、include completion、server routing、parser、query、resolver、coloring 和大测试聚合热点已拆到聚焦子模块。
...
9. 此前的 `parser/ast.rs`、`query.rs`、`server/language_server.rs`、`completion.rs` 等大文件热点已拆分；新增关系分析能力仍应落到聚焦子模块，避免重新把复杂度堆回这些入口文件。
